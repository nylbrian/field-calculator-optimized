...
class HouseholdSurveyWomenEmpowermentDAO {
  const MAIN_TABLE = 'household_survey_women_empowerment';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`household_survey_id`, `womens_decision`,
    ...
    if (!$id) {
      DB::rollback();
      throw new HouseholdSurveyDAOException('Failed to insert women empowerment information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('UPDATE `' . self::MAIN_TABLE . '`
      SET `household_survey_id` = %d, `womens_decision` = %d,
