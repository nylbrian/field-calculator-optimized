...
class HouseholdSurveyGeneralInformationDAO {
  const MAIN_TABLE = 'household_survey_general_information';
  const PROBLEMS_TABLE = 'household_survey_general_information_problems';
    ...
    foreach ($data as $key => $value) {
      if ($value['problem']) {
        DB::queryMaster('INSERT INTO `' . self::PROBLEMS_TABLE . '`
          (household_survey_general_information_id, problem) VALUES
          (%d, "%s")',
  ...
  public function get($seasonLongId) {
    $result = DB::querySlave('
      SELECT * FROM household_survey_general_information WHERE household_survey_id = %d
    ', array($seasonLongId));
  ...
  public function getProblems($id) {
    $result = DB::querySlave('
      SELECT * FROM household_survey_general_information_problems
      WHERE household_survey_general_information_id = %d',
      array($id));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    $sql = 'UPDATE `' . self::MAIN_TABLE . '`
      SET `household_survey_id` = %d, `gender` = %d, `age` = %d, `marital_status` = %d, `literacy` = %d,
