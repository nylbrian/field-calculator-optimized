...
class HouseholdSurveyPesticideApplicationDAO {
  const MAIN_TABLE = 'household_survey_pesticide_application';
  const INFO_TABLE = 'household_survey_pesticide_rice_problems_detail';
  const EQUIPMENT_TABLE = 'household_survey_pesticide_equipment';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`household_survey_id`, `applied_pesticide`, `applied_pesticide_other`,
    ...
    if (!$id) {
      DB::rollback();
      throw new HouseholdSurveyDAOException('Failed to insert pesticide application information');
    }
  ...
  public function saveProblems($data, $id) {
    ...
    $sql = 'DELETE FROM `' . self::INFO_TABLE . '`
      WHERE `household_survey_pesticide_application_id` = %d';
    ...
    if (isset($data['problemsRiceField'])) {
      foreach ($data['problemsRiceField'] as $key => $value) {
        ...
        DB::queryMaster('INSERT INTO `' . self::INFO_TABLE . '`
          (household_survey_pesticide_application_id, name, applied_date, applied, amount, leftover, brand_applied)
    ...
    if (isset($data['otherProblemsRiceField'])) {
      foreach ($data['otherProblemsRiceField'] as $key => $value) {
        ...
        DB::queryMaster('INSERT INTO `' . self::INFO_TABLE . '`
          (household_survey_pesticide_application_id, name, applied_date, applied, amount, leftover, brand_applied, other)
  ...
  public function saveEquipments($data, $id) {
    ...
    foreach ($data as $key => $value) {
      ...
      DB::queryMaster('INSERT INTO `' . self::EQUIPMENT_TABLE . '`
        (household_survey_pesticide_application_id, value)
        VALUES (%d, %d)',
        array(
          $id,
          $key
  ...
  public function getEquipments($id) {
    $result = DB::querySlave('SELECT * FROM `' . self::EQUIPMENT_TABLE . '`
      WHERE `household_survey_pesticide_application_id` = %d', array($id));
  ...
  public function getProblems($id, &$data) {
    $result = DB::queryMaster('
      SELECT * FROM `' . self::INFO_TABLE . '` WHERE `household_survey_pesticide_application_id` = %d',
      array($id)
    );
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `household_survey_id` = %d, `applied_pesticide` = %d, `applied_pesticide_other` = %d,
