...
class HouseholdSurveyPreplantingDAO {
  const MAIN_TABLE = 'household_survey_pre_planting_information';
  const ORGANIC_MATERIALS_TABLE = 'household_survey_pre_planting_information_organic_materials';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`household_survey_id`, `rice_area`, `parcel_has_treatment`, `treatment_name`, `treatment_name_other`,
    ...
    if (!$id) {
      DB::rollback();
      throw new HouseholdSurveyDAOException('Failed to insert pre planting information');
    }
    ...
    foreach ($data as $key => $value) {
      if ($value['checked']) {
        DB::queryMaster('INSERT INTO `' . self::ORGANIC_MATERIALS_TABLE . '`
          (household_survey_pre_planting_information_id, name, amount) VALUES
          (%d, "%s", %d)',
  ...
  public function get($seasonLongId) {
    $result = DB::querySlave('
      SELECT * FROM household_survey_pre_planting_information WHERE household_survey_id = %d
    ', array($seasonLongId));
  ...
  public function getOrganicMaterials($id) {
    $result = DB::querySlave('
      SELECT * FROM household_survey_pre_planting_information_organic_materials
      WHERE household_survey_pre_planting_information_id = %d',
      array($id));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    $sql = 'UPDATE `' . self::MAIN_TABLE . '`
      SET `household_survey_id` = %d, `rice_area` = "%s", `parcel_has_treatment` = %d,
