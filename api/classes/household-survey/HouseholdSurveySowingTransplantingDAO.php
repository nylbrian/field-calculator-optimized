...
class HouseholdSurveySowingTransplantingDAO {
  const MAIN_TABLE = 'household_survey_sowing_transplanting';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`household_survey_id`, `direct_sowing_method`,
    ...
    if (!$id) {
      DB::rollback();
      throw new HouseholdSurveyDAOException('Failed to insert sowing transplanting information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `household_survey_id` = %d, `direct_sowing_method` = %d,
