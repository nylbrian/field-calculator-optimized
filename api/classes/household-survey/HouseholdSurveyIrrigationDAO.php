...
class HouseholdSurveyIrrigationDAO {
  const MAIN_TABLE = 'household_survey_irrigation';
  const PERIOD_TABLE = 'household_survey_irrigation_period';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`household_survey_id`, `water_condition`, `irrigation_count`,
    ...
    if (!$id) {
      DB::rollback();
      throw new HouseholdSurveyDAOException('Failed to insert irrigation information');
    }
  ...
  public function savePeriod($data, $id) {
    ...
    $sql = 'DELETE FROM `' . self::PERIOD_TABLE . '`
      WHERE `household_survey_irrigation_id` = %d';
    ...
    foreach ($data as $key => $value) {
      ...
      DB::queryMaster('INSERT INTO `' . self::PERIOD_TABLE . '`
        (household_survey_irrigation_id, date, irrigated, water_depth, standing_water, source)
        VALUES (%d, "%s", %d, %d, %d, %d)',
        array(
          $id,
          $value['date'],
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `household_survey_id` = %d, `water_condition` = %d, `irrigation_count` = %d,
