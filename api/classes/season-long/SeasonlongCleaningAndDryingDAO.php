...
class SeasonlongCleaningAndDryingDAO {
  const MAIN_TABLE = 'season_long_cleaning_drying';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `dry_threshed_grain`, `labor_count`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert cleaning drying information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `dry_threshed_grain` = %d, `labor_count` = %d,
