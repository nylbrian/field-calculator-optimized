  ...
  public function getNitrogenRecommendations($data) {
    ...
    $result = DB::queryMaster('
      ...
      FROM farmers f
        ...
        INNER JOIN season_long_fertilizer_application_info sfa ON sfa.season_long_fertilizer_application_id = sf.id
    ...
    $result = DB::queryMaster('
      ...
      FROM farmers f
        ...
        INNER JOIN season_long sl ON sl.farmers_id = f.id AND
          sl.farmer_seasons_id = fs.id
        INNER JOIN season_long_weeding_herbicide wh ON wh.season_long_id = sl.id
        INNER JOIN season_long_weeding_herbicide_application wha
          ON wha.season_long_weeding_herbicide_id = wh.id
  ...
  public function getFoodSafety() {
    $result = DB::queryMaster('
      ...
      FROM farmers f
        ...
        INNER JOIN season_long sl ON sl.farmers_id = f.id AND
          sl.farmer_seasons_id = fs.id
        INNER JOIN season_long_food_safety sls ON sls.season_long_id = sl.id
      WHERE f.id = %f AND fs.id = %f', array(
        $this->farmerId,
        $this->farmerSeasonId,
      ));
  ...
  public function getGrainYield() {
    $result = DB::queryMaster('
      ...
      FROM farmers f
        ...
        INNER JOIN season_long sl ON sl.farmers_id = f.id AND
          sl.farmer_seasons_id = fs.id
        INNER JOIN season_long_grain_and_straw_yield pp ON pp.season_long_id = sl.id
      WHERE f.id = %d AND fs.id = %d', array(
        $this->farmerId,
        $this->farmerSeasonId,
      ));
  ...
  public function getPesticideRows($brand, $date, $days, $operator) {
    if ($operator === 1) {
      ...
      FROM farmers f
        ...
        INNER JOIN season_long sl ON sl.farmers_id = f.id AND
          sl.farmer_seasons_id = fs.id
        INNER JOIN season_long_pesticide_application sa ON sa.season_long_id = sl.id
        INNER JOIN season_long_pesticide_rice_problems_detail ss ON ss.season_long_pesticide_application_id = sa.id
      WHERE f.id = %d AND
        fs.id = %d AND
    ...
    $result = DB::queryMaster('
      SELECT COUNT(f.id) as cnt
        FROM farmers f
          ...
          INNER JOIN season_long_computations sc ON f.id = sc.farmers_id
