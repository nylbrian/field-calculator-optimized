...
class SeasonlongHarvestingAndThreshingDAO {
  const MAIN_TABLE = 'season_long_harvesting_threshing';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `harvesting_method`, `crop_harvest_date`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert harvesting and threshing information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `harvesting_method` = %d, `crop_harvest_date` = "%s",
