...
class SeasonlongWeedingHerbicideDAO {
  const MAIN_TABLE = 'season_long_weeding_herbicide';
  const APPLICATION_TABLE = 'season_long_weeding_herbicide_application';
  const WEEDS_TABLE = 'season_long_weeding_herbicide_weeds';
  ...
  public function insert($data, $seasonLongId) {
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert weeding herbicide information');
    }
  ...
  public function saveHerbicideProduct($data, $id) {
    ...
    $sql = 'DELETE FROM `' . self::APPLICATION_TABLE . '`
      WHERE `season_long_weeding_herbicide_id` = %d';
    ...
    foreach ($data as $key => $value) {
      ...
      DB::queryMaster('INSERT INTO `' . self::APPLICATION_TABLE . '`
        (season_long_weeding_herbicide_id, `date`, `name`,
  ...
  public function getHerbicideProducts($id) {
    $return = DB::queryMaster('
      SELECT * FROM `' . self::APPLICATION_TABLE . '` WHERE season_long_weeding_herbicide_id = %d
    ', array($id));
  ...
  public function saveHerbicideWeed($data, $id) {
    ...
    foreach ($data as $key => $value) {
      DB::queryMaster('INSERT INTO `' . self::WEEDS_TABLE . '`
        (season_long_weeding_herbicide_id, `name`) VALUES(%d, "%s")',
        array(
          $id,
          $value['name']
        )
  ...
  public function getHerbicideWeed($id) {
    $return = DB::queryMaster('
      SELECT * FROM `' . self::WEEDS_TABLE . '` WHERE season_long_weeding_herbicide_id = %d
    ', array($id));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `control_weed` = %d,
