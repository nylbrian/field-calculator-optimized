...
class SeasonlongLandPreparationDAO {
  const MAIN_TABLE = 'season_long_land_preparation';
  const OPERATIONS_TABLE = 'season_long_land_preparation_operations';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `crop_establishment`, `land_preparation_started`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert land preparation information');
    }
  ...
  public function saveOperations($data, $id) {
    $sql = 'DELETE FROM `' . self::OPERATIONS_TABLE . '` WHERE `season_long_land_preparation_id` = %d';
    DB::queryMaster($sql, array($id));

    foreach ($data as $key => $value) {
      DB::queryMaster('INSERT INTO `' . self::OPERATIONS_TABLE . '`
        (`season_long_land_preparation_id`, `name`, `date`,
  ...
  public function getOperations($id) {
    $result = DB::queryMaster('SELECT * FROM `' . self::OPERATIONS_TABLE . '`
      WHERE season_long_land_preparation_id = %d
    ', array($id));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `crop_establishment` = %d, `land_preparation_started` = "%s",
