...
class SeasonlongWorkersHealthSafetyDAO {
  const MAIN_TABLE = 'season_long_workers_health_safety';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `work_related_injuries`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert workers health safety information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `work_related_injuries` = %d,
