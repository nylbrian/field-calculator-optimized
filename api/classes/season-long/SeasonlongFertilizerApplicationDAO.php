...
class SeasonlongFertilizerApplicationDAO {
  const MAIN_TABLE = 'season_long_fertilizer_application';
  const INFO_TABLE = 'season_long_fertilizer_application_info';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `fertilizer_cost`, `labor_type`, `male_labor`,
  ...
  public function saveInfo($data, $id) {
    ...
    $sql = 'DELETE FROM `' . self::INFO_TABLE . '`
      WHERE `season_long_fertilizer_application_id` = %d';
    ...
    if (isset($data['fertilizerApplied'])) {
      foreach ($data['fertilizerApplied'] as $key => $value) {
        ...
        DB::queryMaster('INSERT INTO `' . self::INFO_TABLE . '`
          (season_long_fertilizer_application_id, name, date, amount, n, p205, k20, zn, s)
    ...
    if (isset($data['otherFertilizerApplied'])) {
      foreach ($data['otherFertilizerApplied'] as $key => $value) {
        ...
        DB::queryMaster('INSERT INTO `' . self::INFO_TABLE . '`
          (season_long_fertilizer_application_id, name, date, amount, n, p205, k20, zn, s, other)
  ...
  public function getInfo($id, &$data) {
    $result = DB::querySlave('
      SELECT * FROM `' . self::INFO_TABLE . '` WHERE season_long_fertilizer_application_id = %d
    ', array($id));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `fertilizer_cost` = %d, `labor_type` = %d, `male_labor` = %d,
