...
class SeasonlongChildLaborDAO {
  const MAIN_TABLE = 'season_long_child_labor';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `employment_below_18`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert child labor information');
    }
  ...
  protected function insertMinimumWage($id, $data) {
    DB::queryMaster('
      DELETE FROM season_long_child_labor_record WHERE season_long_child_labor_id = %d',
      array($id));
    ...
    foreach($data as $key => $value) {
      ...
      DB::queryMaster('
        INSERT INTO season_long_child_labor_record(season_long_child_labor_id, value)
        VALUES(%d, %d)', array(
          $id,
          $key,
      ));
  ...
  public function update($id, $data, $seasonLongId) {
    ...
    DB::queryMaster('
      ...
      SET `season_long_id` = %d, `employment_below_18` = %d,
