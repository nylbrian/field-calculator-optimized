...
class SeasonlongGrainAndStrawYieldDAO {
  const MAIN_TABLE = 'season_long_grain_and_straw_yield';
  ...
  public function insert($data, $seasonLongId) {
    ...
    $sql = 'INSERT INTO `' . self::MAIN_TABLE . '`
      (`season_long_id`, `rice_amount`, `grain_amount`,
    ...
    if (!$id) {
      DB::rollback();
      throw new SeasonlongDAOException('Failed to insert grain and straw yield information');
    }
  ...
  public function update($id, $data, $seasonLongId) {
    DB::queryMaster('UPDATE `' . self::MAIN_TABLE . '`
      SET `season_long_id` = %d, `rice_amount` = %d, `grain_amount` = %d,
